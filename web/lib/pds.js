...
//
// This client does NOT refresh, and adding a refresh here is not an improvement.
// The server holds a copy of the same session and refreshes it, and there is one
// refresh token between them: whoever rotates second presents a token the first
// already spent, and the PDS answers a replay by killing the session outright --
// a dead door for an account that did nothing wrong. The server is the only
// rotator, because it is the one that has to survive a long-running VM. When this
// page's token ages out it asks the user to sign in again, which deposits a fresh
// session and is the one recovery that cannot race anything.
